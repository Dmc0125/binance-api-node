...
const test = async () => {
  ...
  try {
    ...
    // const candlesticks = await binance.spot.candlesticks('BTCUSDT', '1h', { limit: 50000 });

    // console.log(candlesticks.length, new Date(candlesticks[0].openTime), new Date(candlesticks[candlesticks.length - 1].openTime));
